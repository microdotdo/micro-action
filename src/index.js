...
const CLI_CHECKSUMS = {
  ...
  "0.10.3": "0022cef17ad0f0365c166e65a58df98366572d6a361c055354e480af81a2a2a2",
};
...
async function boundedDownload(url) {
  const response = await fetch(url, { redirect: "follow", headers: { "user-agent": "micro-action/1.4.1" } });
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const response = await fetch(url, {
        method: "GET",
        redirect: "manual",
        headers: { "user-agent": "micro-action/1.4.1 live-verification" },
...
async function main() {
  ...
  const cli = await installCli(input("cli-version", "0.10.3"));
